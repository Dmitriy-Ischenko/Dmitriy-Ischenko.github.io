var ie = ie_version();
var p = {
    navbarScrollTop: $(window).height() / 4,
    scrollSpeed: 1000,
    scrollOffset: 80,
...
var all = {
    ...
    appear: function() {
        $('#features, #features2').find('.row').appear(function() {
        ...
        $('#prices').find('.row').appear(function() {
            ...
            setTimeout(function() {
                ...
                $price.eq(2).addClass('animated fadeInDown');
                $price.eq(3).addClass('animated fadeInRight');
            }, 400);
        });
    ...
    onePageNav: function() {
        ...
        //     easing: 'swing'
        // });
        $('#header').find('.nav li a').click(function(){
            $('.nav li').removeClass('active');
            $(this).parent().addClass('active');
            var scroll =  $( $.attr(this, 'href') ).offset().top- $('#header').outerHeight();
    ...
    iheightInit: {
        features: function() {
            var $feature = $('#features, #features2').find('.feature-item');
            $feature.height('auto');
...
jQuery('#google-map').on('click', function(e) {
    if (jQuery('#google-map').children().length == 0) {
        var s = document.createElement('script');
        s.type = 'text/javascript';
        s.async = true;
        s.src = '//api-maps.yandex.ru/services/constructor/1.0/js/?sid=0MJqG4tjGOPA8tMaNtDCfFhtCcmDbOQQ&width=100%&height=430';
        var x = document.getElementsByClassName('yandex-map-block')[0];
        x.appendChild(s);
    }
});
